...
def getData():
    with urllib.request.urlopen("https://waterservices.usgs.gov/nwis/dv/?format=json&indent=on&stateCd=mi&period=P200W&siteStatus=all") as url:
        data = json.load(url)
        ...
        dataFrame = pd.DataFrame.from_dict(data, orient='index', columns=['DateTime', 'Latitude', 'Longitude', 'Temperature', 'Conductance', 'Dissolved_oxygen', 'PH', 'Turbidity'])

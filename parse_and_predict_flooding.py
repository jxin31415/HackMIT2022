...
def makePredictions(scope, month, day, year):
    ...
    # Discharge to float
    dataset['Discharge,cubic_feet_per_second'] = pd.to_numeric(
        dataset['Discharge,cubic_feet_per_second'])
    dataset.rename(
        columns={'Discharge,cubic_feet_per_second': 'Discharge'}, inplace=True)
    # Water_temp
    ...
    dataset["fld_YN"] = np.where(
        dataset['Gage_height'] >= dataset['fld_stg'], 1, 0)
    ...
    if (scope == "7day_daily"):
        ...
        dataset = dataset[((dataset.datetime.dt.hour == 0) & (
            dataset.datetime.dt.minute == 0)) | (dataset.fld_YN == 1)]
        ...
        dataset['Gage_height'] = datasetOrig['Gage_height'].rolling(
            96).max().shift(-95)
...
# -------------------------------
        ...
        if (scope == "7day_6hrs"):
            ...
            dataset = dataset[(dataset.datetime.dt.hour %
                               6 == 0) | (dataset.fld_YN == 1)]
            dataset = dataset[(dataset.datetime.dt.minute == 0)
                              | (dataset.fld_YN == 1)]
    ...
    dataset = dataset.drop(
        [c for c in dataset if c.endswith('_cd')], 1, errors='ignore')
    ...
    dataset = dataset[['datetime', 'fld_stg'] + [c for c in dataset if c not in ['datetime',
                                                                                 'fld_stg', 'fld_YN', 'Gage_height_shift']] + ['fld_YN', 'Gage_height_shift']]
    ...
    # get indices of rows where Gage_height == fld_stg
    fld_index = dataset.index[dataset['Gage_height']
                              == dataset['fld_stg']].tolist()
    if (len(fld_index) == 0):  # none exactly equal, but minimum difference
        fld_index = dataset.index[abs(dataset['Gage_height'] - dataset['fld_stg']) == min(
            abs(dataset['Gage_height'] - dataset['fld_stg']))].tolist()
    ...
    fld_stgs_scaled = [
        fld_stg_scaled, (0.75 * fld_stg_scaled), (0.5 * fld_stg_scaled), (0.25 * fld_stg_scaled)]
    ...
    df_validation.iloc[-1,
                       df_validation.columns.get_loc('Gage_height')] = fld_stg_value

    # get indices of rows where Gage_height == fld_stg
    fld_index_valid = df_validation.index[df_validation['Gage_height']
                                          == df_validation['fld_stg']].tolist()
    if (len(fld_index_valid) == 0):  # none exactly equal, but minimum difference
        fld_index_valid = df_validation.index[abs(df_validation['Gage_height'] - df_validation['fld_stg']) == min(
            abs(df_validation['Gage_height'] - df_validation['fld_stg']))].tolist()

    fld_index_valid = fld_index_valid[0]
    fld_stg_valid_value = df_validation.iloc[0,
                                             df_validation.columns.get_loc('fld_stg')]
    ...
    df_fldstg_valid = pd.DataFrame(
        {'Gage_height': [min_gh_valid, fld_stg_valid_value, max_gh_valid]})

    fld_stg_valid_scaled = scaler.fit_transform(df_fldstg_valid.values)
    fld_stg_valid_scaled = fld_stg_valid_scaled[1][0]

    fld_stgs_valid_scaled = [fld_stg_valid_scaled, (
        0.75 * fld_stg_valid_scaled), (0.5 * fld_stg_valid_scaled), (0.25 * fld_stg_valid_scaled)]
    stg_colors = ['r', 'tab:orange', 'y', 'g']
    stg_labels = ['Flood Stage', '75%', '50%', '25%']
    ...
    for stgIndex in range(len(fld_stgs_scaled)):
        pyplot.axhline(y=fld_stgs_scaled[stgIndex], color=stg_colors[stgIndex],
    ...
    for stgIndex in range(len(fld_stgs_scaled)):
        axis_test.axhline(
            y=fld_stgs_scaled[stgIndex], color=stg_colors[stgIndex], linestyle='-', label=stg_labels[stgIndex])
    ...
    for stgIndex in range(len(fld_stgs_scaled)):
        axis_valid.axhline(y=fld_stgs_scaled[stgIndex], color=stg_colors[stgIndex],
